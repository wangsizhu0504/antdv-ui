...
import type { DefaultOptionType, InternalFieldNames, ShowSearchType } from '../interface'
  ...
  return computed(() => {
    ...
    function dig(list: DefaultOptionType[], pathOptions: DefaultOptionType[]) {
      list.forEach((option) => {
        ...
        if (!sort && Number(limit) > 0 && filteredOptions.length >= Number(limit))
          return
    ...
    return Number(limit) > 0 ? filteredOptions.slice(0, limit as number) : filteredOptions
  })
}
