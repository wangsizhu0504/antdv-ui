import type { InnerMentionsProps, MentionOptionProps } from './props'
...
export function validateSearch(text: string, props: InnerMentionsProps) {
  const { split } = props
  return !split || !text.includes(split)
}

export function filterOption(input: string, { value = '' }: MentionOptionProps): boolean {
