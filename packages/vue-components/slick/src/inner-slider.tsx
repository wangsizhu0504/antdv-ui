...
export default {
  ...
  methods: {
    ...
    slideHandler(index, dotAnimate = false) {
    ...
    changeSlide(options, dotAnimate = false) {
      const spec = { ...this.$props, ...this.$data }
      const targetSlide = changeSlide(spec, options)
      if (targetSlide !== 0 && !targetSlide) return
      if (dotAnimate === true)
        this.slideHandler(targetSlide, dotAnimate)
      else
    ...
    slickGoTo(slide, dotAnimate = false) {
      slide = Number(slide)
