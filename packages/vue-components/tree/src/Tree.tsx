...
import { KeyCode, classNames, initDefaultProps, pickAttrs, warningFn } from '@antdv/utils'
        ...
        promise
          ...
          .catch((e) => {
            ...
            if (loadingRetryTimes[key] >= MAX_RETRY_TIMES) {
              warningFn(false, 'Retry for `loadData` many times but still failed. No more retry.')
              const newLoadedKeys = arrAdd(loadedKeys.value, key)
              if (props.loadedKeys === undefined)
                loadedKeys.value = newLoadedKeys
