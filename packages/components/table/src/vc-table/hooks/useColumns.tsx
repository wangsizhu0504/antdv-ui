...
function warningFixed(flattenColumns: ReadonlyArray<{ fixed?: FixedType }>) {
  ...
  for (let i = 0; i < flattenColumns.length; i += 1) {
    ...
    if (allFixLeft && col.fixed !== 'left') {
      ...
      warningFn(false, `Index ${i - 1} of \`columns\` missing \`fixed='left'\` prop.`)
      break
    }
  }
  ...
  for (let i = flattenColumns.length - 1; i >= 0; i -= 1) {
    ...
    if (allFixRight && col.fixed !== 'right') {
      ...
      warningFn(false, `Index ${i + 1} of \`columns\` missing \`fixed='right'\` prop.`)
      break
    }
  }
}
  ...
  const withExpandColumns = computed<ColumnsType<RecordType>>(() => {
    if (expandable.value) {
      ...
      if (
        ...
        warningFn(false, 'There exist more than one `EXPAND_COLUMN` in `columns`.')
    ...
    if (process.env.NODE_ENV !== 'production' && baseColumns.value.includes(EXPAND_COLUMN))
      warningFn(false, '`expandable` is not config but there exist `EXPAND_COLUMN` in `columns`.')
