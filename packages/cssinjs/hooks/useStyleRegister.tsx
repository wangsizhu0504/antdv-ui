...
export function parseStyle(
  ...
  if (!root) {
    ...
    // fix: https://github.com/thysultan/stylis/pull/339
    if (styleStr) {
...
// ==                                Register                                ==
// ============================================================================
export function uniqueHash(path: Array<string | number>, styleStr: string) {
...
export default function useStyleRegister(
  ...
  const fullPath = computed(() => {
    const _fullPath = [tokenKey.value];
    if (styleContext.value.layer) {
      _fullPath.push('layer');
    }
    _fullPath.push(...info.value.path);
    return _fullPath;
  });
  ...
  const cacheStyle = useGlobalCache<StyleCacheValue>(
    ...
    () => {
      ...
      const { hashPriority, container, transformers, linters, cache, layer: enableLayer } = styleContext.value;
